...
1. Install FFmpeg.
2. Add FFmpeg to the Windows `PATH`.
3. Verify that both `ffmpeg` and `ffprobe` work from PowerShell.
4. Place `slideshow.ps1` in your media directory, or specify another directory with `-Path`.
...
1. Creates a blurred background.
2. Fits the original media inside the output frame.
3. Centers the original media over the blurred background.
